...
const ValidationQueue: React.FC = () => {
    ...
    useEffect(() => { let count = 0; if (statusFilter !== (activeTab === 'room' ? 'returned' : 'all')) count++; if (dateFilter) count++; setActiveFiltersCount(count); }, [statusFilter, dateFilter, activeTab]);

    const fetchPendingCheckouts = useCallback(async () => {
        try {
            if (!loading) setLoading(true);
            let query = supabase.from('checkouts')
                .select(`*, user:users!checkouts_user_id_fkey(*, phone_number), booking:bookings(*, equipment_requested, room:rooms(*, department:departments(*)))`)
                .eq('type', activeTab);
            ...
            let processedData = (checkoutData || []).map(c => ({
                ...c,
                booking: {
                    ...c.booking,
                    equipment_requested_details: (c.booking?.equipment_requested || []).map(id => equipmentDetailsMap.get(id)).filter(Boolean) as Equipment[]
                }
            }));
