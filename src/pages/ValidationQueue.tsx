...
import {
    ...
    ChevronDown, Flag, AlertCircle as AlertCircleIcon, Calendar as CalendarIcon, Phone
} from 'lucide-react';
...
const ValidationQueue: React.FC = () => {
    ...
    const handleApproval = async (checkoutId: string) => {
        ...
        try {
            ...
            const itemsToInsert = Array.from(checkedItems).map(equipmentId => ({ checkout_id: checkoutId, equipment_id: equipmentId, quantity: 1, condition_notes: conditionNotes[equipmentId] || null, }));
            if (itemsToInsert.length > 0) {
                const { error: insertError } = await supabase.from('checkout_items').insert(itemsToInsert);
    ...
    return (
        <div className="space-y-6">
            <div className="bg-gradient-to-r from-orange-600 to-red-600 rounded-xl p-6 text-white">
                <div className="flex items-center justify-between"><div><h1 className="text-3xl font-bold flex items-center space-x-3"><Bell className="h-8 w-8" /><span>Validation Queue</span></h1><p className="mt-2 opacity-90">Review and validate checkouts</p></div><div className="hidden md:block text-right"><div className="text-2xl font-bold">{checkouts.length}</div><div className="text-sm opacity-80">Items in Queue</div></div></div>
            </div>

            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="flex border-b border-gray-200"><button onClick={() => setActiveTab('room')} className={`flex-1 py-4 px-6 text-center font-medium text-sm transition-colors duration-200 ${ activeTab === 'room' ? 'text-orange-600 border-b-2 border-orange-600' : 'text-gray-500 hover:text-gray-700'}`}>Room Checkouts</button><button onClick={() => setActiveTab('equipment')} className={`flex-1 py-4 px-6 text-center font-medium text-sm transition-colors duration-200 ${ activeTab === 'equipment' ? 'text-orange-600 border-b-2 border-orange-600' : 'text-gray-500 hover:text-gray-700'}`}>Equipment Checkouts</button></div>
            </div>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                        <div className="bg-white p-6 rounded-2xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                            ...
                            <div className="space-y-6 pt-5">
                                ...
                                {activeTab === 'room' && verificationList.length > 0 && (
                                    <div className="border rounded-xl p-4">
                                        <h4 className="text-base font-semibold text-gray-500 mb-3">Equipment Verification</h4>
                                        {!allMandatoryChecked && (
                                            <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-3 rounded-md mb-4 flex items-center space-x-2"><AlertCircleIcon className="h-5 w-5" /><p className="font-bold">All mandatory items must be checked to approve.</p></div>
                                        )}
                                        <div className="space-y-3">
                                            {verificationList.map(eq => (
                                                <div key={eq.id} className="grid grid-cols-[auto,1fr,1fr] gap-x-4 items-center">
                                                    <input type="checkbox" checked={checkedItems.has(eq.id)} onChange={(e) => { setCheckedItems(prev => { const newSet = new Set(prev); if (e.target.checked) newSet.add(eq.id); else newSet.delete(eq.id); return newSet; }); }} className="h-5 w-5 rounded border-gray-300 text-orange-600 focus:ring-orange-500" />
                                                    <label className="text-sm font-medium text-gray-800">{eq.name}{eq.is_mandatory && <span className="text-red-500 font-bold ml-1">*</span>}</label>
                                                    <input type="text" placeholder="Condition notes (optional)..." value={conditionNotes[eq.id] || ''} onChange={(e) => setConditionNotes(prev => ({...prev, [eq.id]: e.target.value}))} className="text-sm border border-gray-300 rounded-md px-2 py-1 w-full focus:ring-orange-500 focus:border-orange-500"/>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                <div className="mt-8 flex justify-end items-center gap-x-3 border-t pt-4">
                                    ...
                                    <button onClick={() => handleApproval(selectedCheckout.id)} disabled={isProcessing || !allMandatoryChecked} className="flex items-center space-x-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50"><Check className="h-4 w-4" /><span>Approve Return</span></button>
                                    <button onClick={() => { setShowDetailModal(false); setShowDeleteConfirm(selectedCheckout.id);}} disabled={isProcessing} className="flex items-center space-x-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50"><X className="h-4 w-4" /><span>Reject Return</span></button>
                                </div>
                            </div>
                        </div>
                    </div>
