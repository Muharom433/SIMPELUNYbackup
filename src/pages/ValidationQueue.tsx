            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div className="mt-6 flex justify-end space-x-3 pt-4 border-t">
                  ...
                  <button onClick={handleAddReport} disabled={!reportTitle || !reportDescription || processingIds.has(selectedCheckout.id)} className="flex items-center justify-center space-x-2 px-4 py-2 bg-yellow-600 text-white rounded-lg disabled:opacity-50"><Flag className="h-4 w-4" /><span>{processingIds.has(selectedCheckout.id) ? 'Saving...' : (selectedCheckout.has_report ? 'Update Report' : 'Add Report')}</span></button>
                </div>
              </div>
            </div>
        )
