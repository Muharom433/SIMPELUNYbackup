...
const ExcelUploadModal: React.FC<ExcelUploadModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const processExcelFile = async (file: File) => {
      ...
      for (const sheetName of workbook.SheetNames) {
          ...
          for (const [index, row] of jsonData.entries()) {
              try {
                  ...
                  if (!day || !start_time || !end_time) { allErrors.push(`Sheet '${sheetName}', Baris ${index + 2}: Format 'Jadwal hari' tidak valid.`); }
                  allTransformedData.push(transformedRow);
              } catch (error: any) { allErrors.push(`Sheet '${sheetName}', Baris ${index + 2}: Error - ${error.message}`); }
          }
      }
    ...
    } catch (error) { console.error('Error processing Excel file:', error); toast.error('Gagal memproses file Excel.'); } finally { setUploading(false); }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      console.log('Menghapus jadwal lama...');
      ...
      for (let i = 0; i < scheduleData.length; i += batchSize) {
        ...
        console.log(`Mengunggah batch ${i / batchSize + 1}...`);
