...
const ProgressSidebar = () => (
    ...
    <div className="space-y-4 md:grid md:grid-cols-1 lg:grid-cols-3 md:gap-6 md:space-y-0">
      <div>
        ...
        <div className="relative">
          ...
          />
          <ChevronDown className="absolute right-3 top-1/2 transform -translate-y-1/2 h-4 w-4 md:h-5 md:w-5 text-gray-400" />
          
          {showStudentDropdown && studentSearch && (
            <div 
              className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-lg md:rounded-xl shadow-xl max-h-48 md:max-h-60 overflow-y-auto"
              ...
              {students
                ...
                .map(student => (
                  <button
                    ...
                    className="w-full text-left px-3 md:px-4 py-2 md:py-3 hover:bg-blue-50 border-b border-gray-100 last:border-b-0 transition-colors duration-150"
                  >
              ...
              {studyPrograms
                ...
                .map(program => (
                  <button
                    ...
                    className="w-full text-left px-3 md:px-4 py-2 md:py-3 hover:bg-blue-50 border-b border-gray-100 last:border-b-0 transition-colors duration-150 text-sm md:text-base"
                  >
                    {program.name}
                  </button>
                ))
 ...
 const RoomAndDetailsStep = () => {
  // Tambahkan useCallback untuk mencegah re-render
  ...
  const handleExaminerChange = useCallback((value) => {
    setExaminerSearch(value);
    form.setValue('examiner', value);
    if (value.length > 0) {
      setShowExaminerDropdown(true);
    }
  }, [form]);

  const handleSecretaryChange = useCallback((value) => {
    setSecretarySearch(value);
    form.setValue('secretary', value);
    if (value.length > 0) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          <button
            ...
            className="w-full px-3 md:px-4 py-2.5 md:py-3 border border-gray-300 rounded-lg bg-white text-left focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 flex items-center justify-between text-sm"
          >
            <span className={form.getValues('room_id') ? 'text-gray-900' : 'text-gray-500'}>
              {form.getValues('room_id') 
                ? availableRooms.find(r => r.id === form.getValues('room_id'))?.name + 
                  ` - ${availableRooms.find(r => r.id === form.getValues('room_id'))?.code} (Kapasitas: ${availableRooms.find(r => r.id === form.getValues('room_id'))?.capacity})`
                  ...
                  {availableRooms
                    ...
                    .map(room => (
                      <button
                        ...
                        className="w-full px-3 py-2.5 text-left hover:bg-blue-50 border-b border-gray-100 last:border-b-0 transition-colors duration-150 focus:bg-blue-50 focus:outline-none"
                      >
                        <div className="font-medium text-gray-900 text-sm">{room.name}</div>
                        <div className="text-xs text-gray-500">{room.code} • Kapasitas: {room.capacity}</div>
                      </button>
                    ))
                  }
                </div>
      ...
      {/* Committee Members - STABILIZED VERSION */}
      <div className="space-y-3">
        <h4 className="text-base font-semibold text-gray-800 flex items-center space-x-2">
            ...
            <div className="relative">
              <input
                ...
                onFocus={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setShowSupervisorDropdown(supervisorSearch.length > 0);
                }}
                onBlur={() => {
                  // Delay untuk memberikan waktu pada click dropdown
                  ...
                  {lecturers
                    ...
                    .map(lecturer => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2.5 hover:bg-blue-50 border-b border-gray-100 last:border-b-0 transition-colors duration-150 text-sm"
                      >
                        {lecturer.full_name}
                      </button>
                    ))
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  handleExaminerChange(e.target.value);
                }}
                onFocus={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setShowExaminerDropdown(examinerSearch.length > 0);
                }}
                onBlur={() => {
                  setTimeout(() => setShowExaminerDropdown(false), 200);
                }}
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  handleSecretaryChange(e.target.value);
                }}
                onFocus={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setShowSecretaryDropdown(secretarySearch.length > 0);
                }}
                onBlur={() => {
                  setTimeout(() => setShowSecretaryDropdown(false), 200);
                }}
