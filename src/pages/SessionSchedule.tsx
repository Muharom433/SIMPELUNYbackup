...
const SessionScheduleProgressive = () => {
  ...
  // STUDENT INFORMATION STEP - FIXED
  // STUDENT INFORMATION STEP - IMPROVED WITH BETTER AUTO-FILL
const StudentInformationStep = () => {
  // State untuk NIM dengan auto-fill yang diperbaiki
  const [nimInput, setNimInput] = useState('');
  const [showNimDropdown, setShowNimDropdown] = useState(false);
  const [isManualEntry, setIsManualEntry] = useState(false);
  
  // State untuk Program Studi - Model Search Dropdown
  const [programSearch, setProgramSearch] = useState('');
  const [showProgramDropdown, setShowProgramDropdown] = useState(false);
  const [selectedProgramDisplay, setSelectedProgramDisplay] = useState('');

  // Sinkronisasi initial state dengan formData
  useEffect(() => {
  ...
  // Filter students untuk dropdown berdasarkan input NIM
  ...
  // Auto-fill HANYA ketika ada exact match dan belum manual entry
  useEffect(() => {
    if (nimInput && nimInput.length >= 5 && !isManualEntry) {
      const exactMatch = students.find(student => 
        student.identity_number === nimInput
      );
      
      if (exactMatch && formData.student_nim !== exactMatch.identity_number) {
        // Auto-fill data
        setFormData(prev => ({
          ...prev,
  ...
  }, [nimInput, students, studyPrograms, form, isManualEntry, formData.student_nim]);
  ...
  const handleNimInputChange = useCallback((value) => {
    ...
    setFormData(prev => ({
      ...prev,
      student_nim: value,
      // Reset data lain jika tidak ada match
    ...
    // Reset student_id jika tidak ada exact match
    if (!students.find(s => s.identity_number === value)) {
      form.setValue('student_id', '');
      setSelectedProgramDisplay('');
    }
    
    // Show dropdown jika ada input dan ada hasil
    setShowNimDropdown(value.length > 0);
    setIsManualEntry(true); // Tandai sebagai manual entry
  }, [students, form]);
  ...
  const handleStudentSelect = useCallback((student) => {
    ...
    setShowNimDropdown(false);
    setIsManualEntry(false); // Reset manual entry flag
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="space-y-4 md:grid md:grid-cols-1 lg:grid-cols-3 md:gap-6 md:space-y-0">
        ...
        {/* NIM INPUT - DENGAN AUTO-FILL YANG DIPERBAIKI */}
        <div>
          ...
          {/* Indikator untuk auto-filled */}
          {nimInput && students.find(s => s.identity_number === nimInput) && formData.student_name && (
        ...
        {/* Study Program - MODEL SEARCH DROPDOWN */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
