...
const RoomManagement: React.FC = () => {
  ...
  // State untuk filter pencarian baru
  ...
  const updateRoomStatuses = useCallback(async (dayToFetch: string, timeToCheck: Date, isManual = false) => {
  ...
  }, [profile, searchStartTime, searchEndTime]);
  ...
  const handleSearch = () => {
    ...
    updateRoomStatuses(searchDay, checkTime, true);
  };
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div><h1 className="text-3xl font-bold flex items-center space-x-3"><Building className="h-8 w-8" /><span>Room Status Dashboard</span></h1><p className="mt-2 opacity-90">View real-time room status or search for future availability</p></div>
          <div className="hidden md:block text-right"><div className="text-2xl font-bold">{rooms.length}</div><div className="text-sm opacity-80">Total Rooms</div></div>
        </div>
      </div>

      <div className="bg-white rounded-xl shadow-sm border p-6 space-y-4">
        <div>
          <h3 className="text-lg font-medium text-gray-900 mb-4">Cari Ketersediaan Ruangan</h3>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        {filteredRooms.length === 0 && (<div className="text-center py-12"><Building className="h-12 w-12 text-gray-400 mx-auto mb-4" /><h3 className="text-lg font-medium text-gray-900">No rooms found</h3><p className="text-gray-600">Try adjusting your search criteria.</p></div>)}
      </div>
