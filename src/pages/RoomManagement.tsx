...
const RoomManagement: React.FC = () => {
  ...
  // --- ADDED: State untuk melacak waktu refresh terakhir ---
  ...
  useEffect(() => {
    if (profile) {
      updateRoomStatuses();
      fetchDepartments();
      // --- UPDATED: Interval diubah menjadi 10 menit ---
      const interval = setInterval(() => updateRoomStatuses(), 10 * 60 * 1000); // 10 menit
  ...
  const handleManualRefresh = () => {
    ...
    toast.success('Refreshing room statuses...');
    updateRoomStatuses();
  }

  const fetchDepartments = async () => { /* ... (fungsi tetap sama) ... */ try { const { data, error } = await supabase.from('departments').select('id, name').order('name'); if (error) throw error; setDepartments(data || []); } catch (error: any) { console.error('Error fetching departments:', error); toast.error('Failed to load departments'); } };
  ...
  const handleEdit = (room: RoomWithDetails) => { /* ... (fungsi tetap sama) ... */ setEditingRoom(room); form.reset({ name: room.name, code: room.code, capacity: room.capacity, department_id: room.department_id, }); setShowForm(true); };
  const handleDelete = async (roomId: string) => { /* ... (fungsi tetap sama) ... */ if (!confirm('Are you sure you want to delete this room?')) return; try { const { error } = await supabase.from('rooms').delete().eq('id', roomId); if (error) throw error; toast.success('Room deleted successfully!'); updateRoomStatuses(); } catch (error: any) { console.error('Error deleting room:', error); toast.error(error.message || 'Failed to delete room'); } };
  const filteredRooms = useMemo(() => { /* ... (fungsi tetap sama) ... */ return rooms.filter(room => { const matchesSearch = (room.name?.toLowerCase() || '').includes(searchTerm.toLowerCase()) || (room.code?.toLowerCase() || '').includes(searchTerm.toLowerCase()) || (room.department?.name?.toLowerCase() || '').includes(searchTerm.toLowerCase()); const matchesStatus = filterStatus === 'all' || room.status.toLowerCase().replace(' ', '_') === filterStatus; return matchesSearch && matchesStatus; }); }, [rooms, searchTerm, filterStatus]);
  const getStatusColor = (status: RoomWithDetails['status']) => { /* ... (fungsi tetap sama) ... */ switch (status) { case 'In Use': return 'bg-red-100 text-red-800'; case 'Scheduled': return 'bg-yellow-100 text-yellow-800'; case 'Available': return 'bg-green-100 text-green-800'; default: return 'bg-gray-100 text-gray-800'; } };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6"> <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between"> <div className="flex flex-wrap gap-3"> <button onClick={() => { setEditingRoom(null); form.reset(); setShowForm(true); }} className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"> <Plus className="h-5 w-5" /> <span>Add Room</span> </button> 
        {/* --- UPDATED: Tombol Refresh sekarang memanggil handleManualRefresh --- */}
