...
const RoomManagement: React.FC = () => {
  ...
  // --- UPDATED: Mengganti 'rooms' menjadi 'allRooms' & 'displayedRooms' ---
      ...
      const roomsWithStatus = roomsData.map(room => {
        ...
        if (roomSchedules.length > 0) {
          const isCurrentlyInUse = roomSchedules.some(schedule => {
            ...
            try { return now >= parse(schedule.start_time, 'HH:mm:ss', new Date()) && now <= parse(schedule.end_time, 'HH:mm:ss', new Date()); } catch (e) { return false; }
          });
    ...
    try {
      ...
      schedulesData.forEach(schedule => {
          if(schedule.room && schedule.start_time && schedule.end_time) {
              ...
              if(scheduleStart < searchEnd && scheduleEnd > searchStart) { busyRoomNames.add(schedule.room); }
          }
      });
      const { data: allRoomsData, error: roomsError } = await supabase.from('rooms').select(`*, department:departments(*)`);
      if(roomsError) throw roomsError;
      const availableRooms = allRoomsData.filter(room => !busyRoomNames.has(room.name)).map(room => ({ ...room, department: room.department, status: 'Available' as 'Available' }));
      setDisplayedRooms(availableRooms as RoomWithDetails[]);
      toast.success(`Found ${availableRooms.length} available rooms.`);
    } catch (error) { console.error('Error searching for available rooms:', error); toast.error('Failed to perform search.');
    } finally { setIsRefreshing(false); setLoading(false); }
  };

  const handleManualRefresh = () => { setIsSearchMode(false); refreshTodayStatus(true); };
  useEffect(() => { if (profile) { refreshTodayStatus(); fetchDepartments(); fetchMasterEquipment(); const interval = setInterval(() => { if (!isSearchMode) { refreshTodayStatus(); } }, 10 * 60 * 1000); return () => clearInterval(interval); } }, [profile, isSearchMode, refreshTodayStatus]);
  ...
  useEffect(() => { if (showRoomDetail) { fetchSchedulesForRoom(showRoomDetail.name, searchDay); fetchRoomEquipment(showRoomDetail.id); } }, [showRoomDetail, searchDay]);
  ...
  // --- UPDATED: This now filters the correct state variable: `displayedRooms` ---
  ...
  return (
    <div className="space-y-6">
        ...
        {showRoomDetail && ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"> <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col"> <div className="p-6 border-b flex justify-between items-center"> <h2 className="text-2xl font-bold text-gray-900">{showRoomDetail.name} ({showRoomDetail.code})</h2> <button onClick={() => setShowRoomDetail(null)} className="p-2 text-gray-400 hover:text-gray-600 rounded-full"><X/></button> </div> <div className="p-6 grid md:grid-cols-2 gap-6 overflow-y-auto"> <div className="space-y-4"> <h3 className="text-lg font-semibold text-gray-800">Manage Equipment</h3> <div className="p-4 bg-gray-50 rounded-lg space-y-2"> {masterEquipmentList.map((eq) => { const Icon = iconMap[eq.icon_name || 'Zap'] || Zap; const isChecked = roomEquipmentLinks.some(link => link.equipment_id === eq.id); const isProcessing = isUpdatingEquipment === eq.id; return ( <label key={eq.id} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm"> <div className="flex items-center space-x-3"> <Icon className="h-5 w-5 text-gray-500" /> <div><span className="font-medium text-gray-800">{eq.name}</span><span className="block text-xs text-gray-500">{eq.category}</span></div> </div> {isProcessing ? <Loader2 className="h-5 w-5 animate-spin text-gray-400"/> : <input type="checkbox" checked={isChecked} onChange={(e) => handleEquipmentChange(eq.id, e.target.checked)} className="h-5 w-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 cursor-pointer"/> } </label> ); })} {masterEquipmentList.length === 0 && <p className="text-sm text-gray-500 text-center py-4">No master equipment data found.</p>} </div> </div> <div className="space-y-4"> <h3 className="text-lg font-semibold text-gray-800">Schedule for {searchDay}</h3> <div className="p-4 bg-gray-50 rounded-lg"> {loadingSchedules ? <div className="flex justify-center items-center h-24"><RefreshCw className="animate-spin h-6 w-6 text-gray-500"/></div> : roomSchedules.length > 0 ? ( <ul className="space-y-2"> {roomSchedules.map(schedule => ( <li key={schedule.id} className="p-3 bg-white rounded-md shadow-sm"> <p className="font-semibold">{schedule.course_name}</p> <p className="text-sm text-gray-600">{schedule.start_time?.substring(0,5)} - {schedule.end_time?.substring(0,5)}</p> <p className="text-xs text-gray-500">Prodi: {schedule.subject_study}</p> </li> ))} </ul> ) : ( <p className="text-sm text-gray-500 text-center py-4">No schedules for this room on the selected day.</p> )} </div> </div> </div> </div> </div> )}
    </div>
  );
};
