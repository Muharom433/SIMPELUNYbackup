...
// --- UPDATED: department_id is now optional ---
...
const RoomManagement: React.FC = () => {
  ...
  const handleManualRefresh = () => {
    ...
    setSearchDay(format(new Date(), 'EEEE'));
  ...
  useEffect(() => {
    if (profile) {
      ...
      const interval = setInterval(() => { if (!isSearchMode) { updateRoomStatuses(); } }, 10 * 60 * 1000);
      return () => clearInterval(interval);
    }
  }, [profile, isSearchMode, updateRoomStatuses]);
  ...
  const onSubmit = async (data: RoomForm) => {
    try {
      ...
      const roomData = {
        ...
        department_id: data.department_id || null, // Allow null
      };
      if (editingRoom) {
  ...
  const filteredRooms = useMemo(() => {
    return (isSearchMode ? rooms : allRooms).filter(room => {
      ...
      <div className="bg-white rounded-xl shadow-sm border p-6 space-y-4">
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <button onClick={findAvailableRooms} disabled={isRefreshing} className="flex items-center justify-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 h-10">{isRefreshing ? <Loader2 className="h-5 w-5 animate-spin"/> : <Search className="h-5 w-5" />} <span>Search</span></button>
          </div>
        </div>
