...
const BookRoom: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 p-4">
          ...
          : viewMode === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredRooms.map((room) => (
                <div key={room.id} className={`p-4 border-2 rounded-lg transition-all ${selectedRoom?.id === room.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}>
                  <div className="flex items-start justify-between mb-3">
                    <div className="cursor-pointer" onClick={() => { setSelectedRoom(room); form.setValue('room_id', room.id); }}>
                  ...
                  {/* --- ADDED: Eye icon for scheduled rooms --- */}
      ...
      {/* Booking Form Column remains unchanged */}
