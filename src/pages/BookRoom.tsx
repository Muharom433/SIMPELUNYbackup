...
// --- UPDATED: Interface now includes dynamic status ---
...
const BookRoom: React.FC = () => {
  ...
  // --- ADDED: Logic to fetch schedules and determine room status ---
  ...
  const fetchStudyPrograms = async () => { try { const { data, error } = await supabase.from('study_programs').select(`*, department:departments(*)`).order('name'); if (error) throw error; setStudyPrograms(data || []); } catch (error) { console.error('Error fetching study programs:', error); toast.error('Failed to load study programs'); } };
  ...
  const getStatusColorAndIcon = (status: RoomWithDetails['status']) => {
    switch (status) {
      case 'In Use': return { bg: 'bg-red-100', text: 'text-red-800', icon: AlertCircle };
      case 'Scheduled': return { bg: 'bg-yellow-100', text: 'text-yellow-800', icon: Clock };
  ...
  const onSubmit = async (data: BookingForm) => {
    ...
    try {
      ...
      updateRoomStatuses();
    } catch (error: any) { console.error('Error creating booking:', error); toast.error(error.message || 'Failed to create booking');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex items-center space-x-2">
                <select value={filterCapacity || ''} onChange={(e) => setFilterCapacity(e.target.value ? Number(e.target.value) : null)} className="px-4 py-3 border border-gray-300 rounded-lg"> <option value="">All Capacities</option> <option value="20">20+ seats</option> <option value="40">40+ seats</option> </select>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            : viewMode === 'grid' ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredRooms.map((room) => {
                  ...
                  return (
                    <div key={room.id} onClick={() => { setSelectedRoom(room); form.setValue('room_id', room.id); }} className={`p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 hover:shadow-md ${selectedRoom?.id === room.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}>
            ...
            ) : ( <div>{/* List View can be added here if needed */}</div> )}
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="space-y-4"> <h3 className="text-lg font-medium text-gray-900 flex items-center space-x-2"> <Calendar className="h-5 w-5" /> <span>Booking Details</span> </h3> <div> <label className="block text-sm font-medium text-gray-700 mb-2"> Start Time * </label> <div className="flex space-x-2"> <input {...form.register('start_time')} type="datetime-local" className="flex-1 px-3 py-2 border border-gray-300 rounded-lg" /> <button type="button" onClick={handleNowBooking} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium"> NOW </button> </div> {form.formState.errors.start_time && ( <p className="mt-1 text-sm text-red-600"> {form.formState.errors.start_time.message} </p> )} </div> <div className="grid grid-cols-2 gap-4"> <div> <label className="block text-sm font-medium text-gray-700 mb-2"> SKS (Credit Hours) * </label> <input {...form.register('sks', { valueAsNumber: true })} type="number" min="1" max="6" className="w-full px-3 py-2 border border-gray-300 rounded-lg" /> {form.formState.errors.sks && ( <p className="mt-1 text-sm text-red-600"> {form.formState.errors.sks.message} </p> )} </div> <div> <label className="block text-sm font-medium text-gray-700 mb-2"> Class Type * </label> <select {...form.register('class_type')} className="w-full px-3 py-2 border border-gray-300 rounded-lg"> <option value="theory">Theory (50 min/SKS)</option> <option value="practical">Practical (170 min/SKS)</option> </select> </div> </div> {form.watch('start_time') && watchSks && ( <div className="bg-green-50 border border-green-200 rounded-lg p-4"> <div className="flex items-center space-x-2"> <Clock className="h-5 w-5 text-green-600" /> <div className="text-sm text-green-800"> <p className="font-medium">Calculated Duration: {watchClassType === 'theory' ? watchSks * 50 : watchSks * 170} minutes</p> {calculateEndTime(form.watch('start_time'), watchSks, watchClassType) && ( <p>End Time: {format(calculateEndTime(form.watch('start_time'), watchSks, watchClassType)!, 'MMM d, yyyy at h:mm a')}</p> )} </div> </div> </div> )} </div>
              <div className="space-y-4"> <h3 className="text-lg font-medium text-gray-900 flex items-center space-x-2"> <Zap className="h-5 w-5" /> <span>Equipment</span> </h3> {mandatoryEquipment.length > 0 && ( <div> <h4 className="text-sm font-medium text-gray-700 mb-3">Mandatory Equipment (Auto-selected)</h4> <div className="space-y-2"> {mandatoryEquipment.map((eq) => { const IconComponent = getEquipmentIcon(eq.category); return ( <div key={eq.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg"> <input type="checkbox" checked={true} disabled={true} className="h-4 w-4 text-blue-600 border-gray-300 rounded" /> <IconComponent className="h-5 w-5 text-gray-600" /> <div className="flex-1"> <p className="text-sm font-medium text-gray-900">{eq.name}</p> <p className="text-xs text-gray-500">{eq.code} • {eq.category}</p> </div> <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800"> Required </span> </div> ); })} </div> </div> )} {optionalEquipment.length > 0 && ( <div> <h4 className="text-sm font-medium text-gray-700 mb-3">Optional Equipment</h4> <div className="space-y-2"> {optionalEquipment.map((eq) => { const IconComponent = getEquipmentIcon(eq.category); return ( <div key={eq.id} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50"> <input type="checkbox" value={eq.id} {...form.register('equipment_requested')} className="h-4 w-4 text-blue-600 border-gray-300 rounded" /> <IconComponent className="h-5 w-5 text-gray-600" /> <div className="flex-1"> <p className="text-sm font-medium text-gray-900">{eq.name}</p> <p className="text-xs text-gray-500">{eq.code} • {eq.category}</p> </div> <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800"> Available </span> </div> ); })} </div> </div> )} </div>
              <div> <label className="block text-sm font-medium text-gray-700 mb-2"> Additional Notes (Optional) </label> <textarea {...form.register('notes')} rows={3} placeholder="Any special requirements, setup instructions, or additional information..." className="w-full px-3 py-2 border border-gray-300 rounded-lg" /> </div>
          ...
          {selectedRoom && ( <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6"> <h3 className="text-lg font-semibold text-gray-900 mb-4">Selected Room</h3> <div className="space-y-3"> <div className="flex items-center justify-between"> <span className="text-sm font-medium text-gray-700">Room:</span> <span className="text-sm text-gray-900">{selectedRoom.name} ({selectedRoom.code})</span> </div> <div className="flex items-center justify-between"> <span className="text-sm font-medium text-gray-700">Department:</span> <span className="text-sm text-gray-900">{selectedRoom.department?.name}</span> </div> <div className="flex items-center justify-between"> <span className="text-sm font-medium text-gray-700">Capacity:</span> <span className="text-sm text-gray-900">{selectedRoom.capacity} seats</span> </div> <div className="flex items-center justify-between"> <span className="text-sm font-medium text-gray-700">Status:</span> <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${selectedRoom.is_available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}> {selectedRoom.is_available ? 'Available' : 'In Use'} </span> </div> {!selectedRoom.is_available && ( <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mt-2"> <div className="flex items-start space-x-2"> <AlertCircle className="h-5 w-5 text-yellow-600 mt-0.5" /> <div className="text-sm text-yellow-800"> <p className="font-medium">Room is currently in use</p> <p>Booking this room will mark previous approved bookings as completed.</p> </div> </div> </div> )} </div> </div> )}
        </div>
      </div>
