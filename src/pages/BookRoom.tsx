      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 opacity-90">Book a room for your lecture, meeting, or study session</p>
          </div>
          <div className="hidden md:block text-right">
            <div className="text-2xl font-bold">{format(currentTime, 'HH:mm')}</div>
            {/* Removed localeID to default to English */}
                 ...
                 {filteredRooms.map((room) => (
                  <div key={room.id}
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      {room.status === 'Scheduled' && (
                        <button title="View Schedule" onClick={(e) => { e.stopPropagation(); setViewingSchedulesFor(room); }} className="p-2 text-gray-400 hover:text-blue-600 rounded-full hover:bg-gray-100">
                            <Eye className="h-5 w-5"/>
                        </button>
                      )}
                    </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        {form.watch('start_time') && watchSks && ( <div className="bg-green-50 border border-green-200 rounded-lg p-4"><div className="flex items-center space-x-2"><Clock className="h-5 w-5 text-green-600" /><div className="text-sm text-green-800"><p className="font-medium">Estimated Duration: {watchClassType === 'theory' ? watchSks * 50 : watchSks * 170} minutes</p>{calculateEndTime(form.watch('start_time'), watchSks, watchClassType) && ( <p>End Time: {format(calculateEndTime(form.watch('start_time'), watchSks, watchClassType)!, "MMM d, yyyy 'at' HH:mm")}</p> )}</div></div></div> )}
                    </div>
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[80vh] flex flex-col">
                ...
                <div className="p-6 overflow-y-auto">
                    {loadingSchedules ? (
                        ...
                        <p className="text-sm text-gray-500 text-center py-4">No schedule found for this room today.</p>
                    )}
                </div>
            </div>
        </div>
