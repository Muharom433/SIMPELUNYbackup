...
const BookingManagement: React.FC = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      console.log('Fetching bookings for user:', profile?.role, profile?.department_id);
      ...
      console.log('Raw bookings fetched:', bookingsData?.length);
      ...
      const bookingsWithDetails = await Promise.all(
        bookingsData.map(async (booking) => {
          ...
          // Fetch user data - use maybeSingle() instead of single()
          if (booking.user_id) {
            try {
          ...
          // Fetch room data - use maybeSingle() instead of single()
          if (booking.room_id) {
            try {
      ...
      if (profile?.role === 'department_admin' && profile.department_id) {
        ...
        console.log('Department filtered bookings:', filteredBookings.length);
      }

      setBookings(filteredBookings);
      console.log('Final bookings set:', filteredBookings.length);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 opacity-90">
              Manage all room bookings and approvals
              {profile?.role === 'department_admin' && ' for your department'}
            </p>
          </div>
                    ...
                    <tr key={booking.id} className="hover:bg-gray-50 transition-colors duration-200">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-3">
                            ...
                            <div className="text-sm text-gray-500">
                              {booking.user?.identity_number || 'N/A'}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          ...
                          <div className="text-sm text-gray-500">
                            {booking.room?.code || 'N/A'} • {booking.room?.department?.name || 'No Department'}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          {booking.status === 'pending' && (
                            <>
                              <button
                                ...
                                className="text-green-600 hover:text-green-900 p-1 rounded transition-colors duration-200 disabled:opacity-50"
                              >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  <h4 className="text-lg font-medium text-gray-900 mb-3">
                    {selectedBooking.purpose || 'Class/Study Session'}
                  </h4>
                  ...
                  <div className="bg-blue-50 rounded-lg p-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-600">{selectedBooking.user?.identity_number || 'N/A'}</div>
                        <div className="text-sm text-gray-600">{selectedBooking.user?.email || 'N/A'}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {selectedBooking.equipment_requested && selectedBooking.equipment_requested.length > 0 && (
                  <div>
                    <h5 className="font-medium text-gray-900 mb-3">Requested Equipment</h5>
                    <div className="space-y-2">
                      {selectedBooking.equipment_requested.map((equipment, index) => (
