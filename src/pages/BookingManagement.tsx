// Updated BookingManagement.tsx with real-time notifications
...
const BookingManagement: React.FC = () => {
    ...
    const { showNotificationToast } = useRealtimeNotifications({
    ...
    const handleStatusUpdate = async (bookingId: string, newStatus: 'approved' | 'rejected') => {
        try {
            ...
            showNotificationToast(`Booking ${newStatus}`, newStatus === 'approved' ? 'success' : 'warning');
            await fetchBookings();
    ...
    const handleDelete = async (bookingId: string) => {
        try {
            ...
            showNotificationToast('Booking deleted', 'warning');
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl p-6 text-white">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-2 opacity-90">Manage all room bookings and approvals{profile?.role === 'department_admin' && ' for your department'} • Real-time updates enabled</p>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[ 
                    { label: 'Pending', count: bookings.filter(b => b.status === 'pending').length, color: 'bg-yellow-500', icon: Clock }, 
                    { label: 'Approved', count: bookings.filter(b => b.status === 'approved').length, color: 'bg-green-500', icon: CheckCircle }, 
                    { label: 'Rejected', count: bookings.filter(b => b.status === 'rejected').length, color: 'bg-red-500', icon: XCircle }, 
                    { label: 'Completed', count: bookings.filter(b => b.status === 'completed').length, color: 'bg-blue-500', icon: Check } 
                ].map((stat, index) => (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center">
                                        <div className="text-gray-500">
                                            ...
                                            {bookings.length > 0 && (
                                                <p className="text-sm mt-2">Total bookings in database: {bookings.length}</p>
                                            )}
                                        </div>
                                    </td>
                                </tr>
                                        ...
                                        <tr key={booking.id} className="hover:bg-gray-50 transition-colors duration-200">
                                                ...
                                                <div>
                                                    ...
                                                    {booking.equipment_requested && booking.equipment_requested.length > 0 && (
                                                        <div className="flex items-center mt-1">
                                                            ...
                                                            <span className="text-xs text-gray-500">{booking.equipment_requested.length} equipment requested</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div className="flex items-center justify-end space-x-2">
                                                    <button onClick={() => { setSelectedBooking(booking); setShowDetailModal(true); }} className="text-gray-600 hover:text-gray-900 p-1 rounded transition-colors duration-200">
                                                        <Eye className="h-4 w-4" />
                                                    </button>
                                                    {booking.status === 'pending' && (
                                                        <>
                                                            <button onClick={() => handleStatusUpdate(booking.id, 'approved')} disabled={isProcessing} className="text-green-600 hover:text-green-900 p-1 rounded transition-colors duration-200 disabled:opacity-50">
                                                                <Check className="h-4 w-4" />
                                                            </button>
                                                            <button onClick={() => handleStatusUpdate(booking.id, 'rejected')} disabled={isProcessing} className="text-red-600 hover:text-red-900 p-1 rounded transition-colors duration-200 disabled:opacity-50">
                                                                <X className="h-4 w-4" />
                                                            </button>
                                                        </>
                                                    )}
                                                    <button onClick={() => setShowDeleteConfirm(booking.id)} disabled={isProcessing} className="text-red-600 hover:text-red-900 p-1 rounded transition-colors duration-200 disabled:opacity-50">
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
                        ...
                        <div className="flex space-x-3">
                            <button onClick={() => setShowDeleteConfirm(null)} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                                Cancel
                            </button>
                            <button onClick={() => handleDelete(showDeleteConfirm)} disabled={processingIds.has(showDeleteConfirm)} className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200">
